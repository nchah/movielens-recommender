...
def s1(input_file):
	...
	for ln in ratings:  # range(0, len(ratings)):
		...
		if pair in input_movie_and_ratings and ln2[0] not in input_user:
			with open("a1-ratings-extract.csv", "a") as f:
				f.write(ln)
...
def s2():
	...
	Sort the users that have the most movie-rating tuples agreement with the input user
	"""
	data = open("a1-ratings-extract.csv", "r").readlines()
	ratings_count = 1
	ratings_count_list = []
	...
	for n in range(0, len(data)):
		...
		if user_id1 != user_id2:
			...
			with open("a2-ratings-extract-counts-by-user.csv", "a") as f:
	...
	output = open("a2-ratings-extract-counts-by-user.csv", "r").readlines()
	...
	for i in count_dict:
		with open("a2-ratings-extract-counts-by-user-distrib.csv", "a") as f:
...
def s3(lower_limit=6, upper_limit=8):
	...
	Get the userIDs with high agreement values
	"""
	data = open("a2-ratings-extract-counts-by-user.csv", "r").readlines()
	agreeing_users = []
	...
	for row in agreeing_users:
		with open("a3-userids-extract-agreeing-users.csv", "a") as f:
			f.write(row + '\n')
...
def s4():
	...
	for ln in ratings:
		...
		if ln2[0] in agreeing_users and pair not in input_movie_and_ratings:
			with open("a4-ratings-extract-recommended.csv", "a") as f:
				f.write(ln)
...
def s5():
	...
	"""
	data = []
	with open("a4-ratings-extract-recommended.csv", "rb") as f:
		for row in csv.reader(f):
			data.append(row)
	data.sort(key=operator.itemgetter(1))  # Column to sort
	with open("a5-ratings-extract-recommended-sorted.csv", "wb") as f:
	...
	After above sorting, get counts of # of movie-rating tuples for each movie
	"""
	output = open("a5-ratings-extract-recommended-sorted.csv", "r").readlines()
	count_dict = {}
	for n in range(0, len(output)):
		row1 = output[n].split(",")
		count1 = int(row1[1].strip())
	...
	for i in count_dict:
		with open("a5-ratings-extract-recommended-sorted-counts.csv", "a") as f:
			f.write(str(i) + ',' + str(count_dict[i]) + '\n')
...
def s6(min_ratings=10):
	...
	"""
	min_rated_movies = []
	with open("a5-ratings-extract-recommended-sorted-counts.csv", "rb") as f:
		for row in csv.reader(f):
	...
	Average the ratings per movie
	"""
	extracted_ratings = open("a5-ratings-extract-recommended-sorted.csv", "r").readlines()
	...
	for n in range(0, len(extracted_ratings)):
		...
		if movie1 in min_rated_movies and movie2 != movie1:
			...
			with open("a6-movies-extract-recommended.csv", "a") as f:
...
def s7(min_avg_rating=4.0):
	...
	rec_data = open("a6-movies-extract-recommended.csv", "r").readlines()

	with open("a7-movies-extract-recommended-films.csv", "a") as f:
